...
- **⚠ 当前状态(2026-06-24,覆盖上面若干旧描述)**:
  ...
  - **🟢 结论已重做成「分角色行动方案」(2026-06-30,替换旧 行动建议+责任归属 iframe 页)**:用户要求结论部分**详细到能推动改进、讲完别让人懵**。现结构 = **三角色总览 1 页 + 3 个角色**(`体验部`紫 / `资料内容`青 / `开发`蓝,把旧 6 部门并成 3)× **8 个板块**:体验部=官网与落地/下载与账号/学习与上手/智能化触点;资料=文档与版本/社区与内容;开发=机器可读出口/工具与工程。旧 `ar-embed.html?sec=actions`、`ar-embed.html?sec=dept&pg=1/2` 三页已从 deck 删除,但历史 embed 能力可保留。
    - **版式方案 B(用户拍板,后续微调)**:每个板块页是「左侧决策摘要 + 右侧优先级行动板」,不是平均清单。`.rolewrap` 两列:左 30% `.rgoal` 玻璃决策卡,右 70% `.ameasures` 行动板。`.rgoal` 顶部用 `data-board` 显示 `01 · 官网与落地` 这类序号+板块名,**不要写“本页拍板”**;`.rgoal` 不要左描边,右下角用 `--rg-icon` 的淡线性语义图形(入口/下载/学习/搜索/文档/社区/代码/工具),不要再用纯圆。**外层只按优先级分组**:`.ahead.p0/p1/p2` 文案只显示 `P0/P1/P2`,不要横线、不要“先拍板/接着做/后续补齐”等口语;`.rmi:has(.p0)` 横跨两列做大卡,P1/P2 做次级卡。**对象维度只能做卡内小标签**:`.who2.hu`=`开发者`、`.who2.co`=`开发者 + AI`、`.who2.ai`=`AI`,不要再把“给开发者 / 共通 / 给 AI”放到外层标题。为让 CSS 根据角色换色,每个 `.rolewrap` 内放一个隐藏 `.role-chip exp/doc/dev`;页头的 `.role-chip` 仍负责可见角色标签。页头 `.bk-strip` 也跟角色走色:`.head-l:has(.role-chip.exp/doc/dev)` 设置 `--tab-role/--tab-soft`,当前 `.bk.on` 用角色实色,其它 `.bk` 用角色淡底。
