...
**7 步生命周期**(`STEPS`,id `L1..L7`):1 落地·找入口·选型 → 2 上手引导 → 3 配环境·拿 Toolkit → 4 动手写·查文档 → 5 编译跑通 → 6 调试·求助 → 7 进阶·跟生态(有经验后)。
...
- ⚠️ **进行中的内容 pass**:触点卡的散文结论(`c[]`/`rec`/`win`)+ 锚点要逐卡校订到"同维度 + 客观"。
  ...
  - **审计结论**:其余卡(3.3/3.4/5.3/5.4/4.4 等)散文**已是"取舍"框架、昇腾长板有 surface**,不属偏向;`4.2 登录`/`4.3 Toolkit下载`的低分是**真实谷底**(登录前置+7步),非偏向,保留。`win` 维持 1.3/3.2/6.3(真实昇腾占优),1.1 现 78/77 但文案"落地都不弱"故不标 win。
...
- **实测表 `MVAL`**:`{触点id:{a:{指标:值}, nv:{...}}}`,**值由 AGENTS.md 取证 + 触点卡描述推定**(非凭空)。**缺测=该维度不适用此触点 → 排除出均值**(不填值)。**28 触点全部已铺**。
...
> **第 4 个一级 tab**`社区承接`(`showTab('comm')`,按钮 `data-t="comm"`,橙 `#e8833a`;`else` 分支自动接管显示 `#tab-comm`)。**与算子/训练 tab 同皮**:每条问题 = 一串「模拟真人找答案」步骤卡(昇腾左真图+注解 / NVIDIA 右),**优先官方、找到就停、找不到转第三方**;末尾给两侧综合承接分。

- **问题源**:算子开发人机协同时序图的 6 条问题(D 写kernel / M Tiling / X 内存越界 / O 注册集成 / L 精度排查 / 决策 soc版本),见 `community_uptake_questions.md`(自包含口径)。
- **打分=与 DJFI 同构**(实测→归一化→合成,**不是另一套**):每目的地实测 5 指标(②可抓取/③详尽/④版本清晰/⑤数量/⑥可信度,1–5)→ nm=v/5 → **综合=1−(1−OFF)(1−SEC)**(OFF=官方②×③、SEC=第三方⑤×⑥)× 版本因子`0.7+0.3·nm④`。官方文档/仓/论坛属官方、不计入第三方。第三方拆**六平台**(CSDN/博客园/知乎/华为云bbs/segmentfault/云厂商;知乎反爬403)。
...
- **6 题昇腾侧已铺满**(真图+注解+承接分):**D 77 / M 78 / X 70 / O 78 / L 77 / 决策 77**。承接分=能否找到可用答案(≠任务难易)。
  - **X 最低 70**:msSanitizer 小众、**第三方几乎只返回官方**(SEC 0.24)→ 没社区兜底,全靠官方,SPA 受阻代价最大。
...
- **NVIDIA 侧已整合**(用户在真浏览器补完 15 张截图,我用 click-annotation skill 加箭头注解、按同口径打分填进右栏):**D 90 / M 96 / X 93 / O 90 / L 85 / 决策 96**。
  - **共性**:NVIDIA 官方文档**全部静态可抓 + 版本清晰**(无 SPA),SO 第三方普遍厚 → 6 题承接分全面高于昇腾。
  - **L 差距最小(85,昇腾 77)**:SO「numerical precision mismatch」该检索词命中 0(措辞偏窄·非真空,cuda-gdb 仍覆盖);昇腾 L 第三方反而厚(CSDN/华为云有量),拉近差距。
...
## CHI 协同研究：两条既有基线（2026-08，勿重复当作参与者实验）

CHI 论文的协同研究不是从零再测“AI 可用性”或“开发者基础体验”。两者均已有独立研究和可复用的原始交付物；新的 20 人研究要观察的是**开发者在两条知识通道之间如何与 Agent 协同、核验、接管与回灌现场事实**。
...
由 **report-ppt-skill** 产出的单文件纵向翻页 deck(13 页),把**两个项目合成一份**:本项目(开发者侧 UX 体验旅程)⊕ `../opknow`(AI 侧「大模型对昇腾内容可获取性」)。主线:**Agent 时代,用户既包括开发者也包括 AI**。封面标题=**「业界大模型对昇腾社区内容可获取性分析」**。**只用算子开发场景**(已去训练)。
...
- **页3「按关系讲解」高亮动效**(用户要的不是横向时间带、而是**竖向按生命线对**:讲矩阵评分=高亮 AI↔昇腾/第三方 的所有箭头、其余压暗;讲技术体验=高亮 人↔昇腾社区 的线)。在 `seq-d3.html` 里做:`renderV()` 给每条消息元素打 `data-pair`(`[frm,to].sort().join('-')`,如 `ai-off`/`ai-sec`/`hu-off`/`hu-sec`/`ai-hu`)、每条生命线打 `data-lane`(BRANCH 人独立分支线 pair=`hu-`+目标)。末尾 embedfix 脚本建右上控制条 `#seqctrl`(‹ / ▶按关系讲解 / ›)+ `applyHi()`:遍历 `#journey` 直接子元素——命中 `GROUPS[step].pairs` 的 `data-pair`→opacity 1、命中涉及 lane 的 `data-lane`→1、其余(决策/自身节点/阶段卡/无标记)→0.1~0.12;step 0=全亮(清 inline opacity)。`GROUPS`=[AI↔昇腾/第三方·矩阵评分, 人↔昇腾社区·技术体验](2 组),循环 0→1→2→0。重绘(滑块)后 renderV 回调 `__seqApplyHi()` 回填。**没绑方向键**(避免和 deck 翻页冲突),只点按钮;蒙层不挡 hover(改的是 opacity、不加遮罩,`pointer-events` 不变)。
- **深挖拆成「左图右内容」专页(用户最终要的形态)**:不是挤在一页右栏,而是**每个关系各拆几页,左侧统一是打着对应聚光灯的时序图、右侧是该页内容**。`seq-d3.html` 加 `?hi=N` 参数(`location.search` 读 → `go(N)` 锁定聚光灯 + 隐藏控制条),deck 各页 `<iframe src="seq-d3.html?hi=1/2">` 内嵌。左栏卡 **统一宽 `flex:0 0 52%`**(不一致会显得图大小不一)。**2026-06 从 36% 加宽到 52%**:36% 时图 SVG(1255×1182,近正方形)被缩到 ~0.35×、11px 字渲染成 ~4px 看不清(且左栏竖长、图下还空一片);52% 缩放升到 ~0.5×、字约大 1.5 倍可读,右栏(深挖页文字少 / 总览两张 `.ovm` 卡)仍放得下。改宽度只改 `index.html` 里 6 处 `flex:0 0 52%`(第3-8页左栏卡)。
  - **AI↔昇腾/第三方(hi=1)** 5 页:① 方法论+场景(怎么跑/7阶段26任务/算子涉及 D·M·X·O·L)② **AI 执行架构**(⚠**2026-06 顺序提到方法论之前**——时序总览→AI怎么跑→怎么测;⚠**此页改版**:`arch-d3` 当**主图(左 64% 卡)** + 右侧「如何运行 / 三种收场:①收敛 ②遇阻回路 ③凭记忆杜撰」文字面板,**去掉了原左侧 `seq-d3`**——因第3页已是整张时序图,架构页再放 seq-d3 会变「两张时序图」重复,用户打回。原描述:引 opknow 20 §4.3 的 Agent 循环图,**已重画**(详见下「当前状态」AI 执行架构图条):一次检索的内部流程——三来源(官方/第三方/自带)→④证据是否充分→充分则⑥收敛终答 / 不足则「能否继续检索」(未达上限且预期有效则换词回③再检索)→检索穷尽且无可靠证据时模型凭记忆杜撰。`arch-d3.html`=cp opknow 20 后注入 embedfix 只露 `#archView`、隐藏 tabs/代码、保留 `#tip`,**hover 详情可用**——与 seq-d3 同套裁法)③ **CUDA vs CANN 问题对**(5 个问句对,左 CUDA 绿/右 CANN 红,原文取自 opknow 18 §2)④ **指标怎么算的**(三源官方/第三方/自带 × 两关触达/质量 → 11 项 `.mxg` 网格 + ⑧成本/⑨⑩产出/⑪综合;**⑪=噪声-OR、一条渠道扛住就能答、受阻记0**,口径取自 opknow 18 §3/§4 `score_metrics.py`)⑤ **§9 重画矩阵**(算子 5 任务×6 维,**每格昇腾大字配色 / NVIDIA「N x」小字**,D 官方可抓=受阻红;数据从 opknow 17 `ROWS` 提取 cann/cuda 1–5 分)。
...
- **另产出 skill**:`~/.Codex/skills/click-annotation/`(把「截图上加点击注解」做成通用 skill,`annotate.py` 颜色/裁剪/场景全参数化,50% 透明圆点+箭头+底部标签条,原图绝不改)。
- **⚠ 当前状态(2026-06-24,覆盖上面若干旧描述)**:
  - **deck = 22 页(2026-06 拆页,含封面;开发者侧 iframe 嵌主报告见末条)**:**封面(双视角标题**:主标「昇腾社区可用性·双视角分析 / 开发者侧基础体验 × AI侧内容可用性」+ kicker/引语同步校平衡;原单写 AI 侧「内容可获取性」过窄) / **立论(head 01 为什么双视角)**=AI 采用三卡(90/80/97),底部 bridge 只留一句「但开发者真的信任 AI 给出的答案吗?」勾子 / **信任落差(head 02,新拆出独立页)**=把 **Stack Overflow 2025** 不信任数据(33%信任 / 46%不信任 / 66%被「看似正确、实则有误」误导)用**同款 `dh-card` 三卡**呈现(原先挤在立论页底 `.bridge.lite` 一行,用户要求单独成页;三卡 ic 用线性 SVG check/x/triangle,数据源全标 Stack Overflow 2025)。**整页红色警示主题**(section 加 `.warn` class scope):因内容=突出问题,改用 skill 状态红 `--g-red`(`#FF5A6E→#FF6F55`)系——**刻意避开品牌红 `#C7000B`**(那是昇腾色),`.warn` 覆盖 dh-num 渐变/dh-ic 红圈/dh-src 红字/accent 卡红渐变/dark 卡深红/bridge 浅红底;scope 在 `.warn` 不动立论页(#2)共用的 dh-card / 时序图总览(head 03) / **AI 侧 5 页**(**执行架构·方法论**·问法·指标·矩阵——架构在前:时序总览→AI 怎么跑→怎么测) / **开发者侧 11 页**(指标定义·方法·评分 + iframe 嵌主报告:旅程 / 5 条综述 / 行动建议 / 责任归属,见末条) / **结论 2 页**(见下条)。〔原 #12+ 的「两套度量/谷底/人类旅程/撞点/改进/A–F/甘特/收尾」早先删过,现按下条重新补了结论。〕
  - **AI 执行架构图(`arch-d3.html`,AI 侧第 2 页)= 重画 + 「按环节讲解」动效**:重画补 **⑦模型自带知识**第三来源(与矩阵三来源对齐)、写明继续检索/停搜/**凭记忆杜撰**的触发条件、`web_search` 标「官方+第三方一并返回」、终答改中性色、删「老实做法」改「模型无法识别自身不知道→凭记忆杜撰」;右上 `‹▶›` 控制条 **7 拍逐环节聚光**(高亮当前节点 / 其余压暗),末拍点出「无可靠证据→凭记忆杜撰」风险。⚠ **讲解动效驱动 deck 外层说明、不是 iframe 内**:arch-d3 每拍 `postMessage({__archCap})` 给父页,`index.html` 监听后更新右卡 `.cmp-cap`——原先更新的是 iframe 内被 embedfix 裁掉的说明行,故看不到变化(踩过的坑)。
  - **AI 侧指标页(「指标如何计算」)**:①②③④ 拆成**一指标一格**、每格悬浮弹出 `score_N` 打分档(取自 opknow 18 §3);补 ⑧⑨⑩(过程/产出);⑪综合重写为 K×版本因子×成本因子、标明 OFF/SEC/OWN 三源构成。
  - **开发者侧页序 = 指标定义 → 方法论 → 评分**(指标即评分标准,排在「右侧已带得分」的方法论/评分前面)。
  - **翻页 = cut 瞬切**(slide `position:fixed` 堆叠 + `.active` 切换,去 scroll-snap;键盘/滚轮/触摸/`#N`/概览/全屏都在;**不是** fade,用户最后选了 cut)。
  - **矩阵(页8)= 重建 + 逐格核对**:布局 = 第1列「检索问题」(`1-5` 阿拉伯编号 + 任务名,`grid-row:span 2` 跨两行,**编号徽章按综合分状态配色** 红<.60/金<.70/绿)+ 第2列 `CANN`/`CUDA` 栈标签 + 6 维等大格;**数据按 opknow `ROWS` 逐格核对**(曾把 D 的「受阻」错放「官方可抓」列——其实在「正文详尽」、D 官方可抓=2);**综合分两侧都必须用 opknow `confCompute()` 公式复算具体小数**(如 D CUDA .88、M .94、X .88、O .84、L .84),不要再写笼统「高」;`d(v)` 把 1-5→[display,score×20],受阻→[受阻,0]。
  - **矩阵后新增全量矩阵页(2026-06-30,2026-07-01 调整)**:算子子集矩阵后加一页 `opknow 全量矩阵`,展示 26 个任务 × 11 指标,每任务 CUDA/CANN 两行,综合列同样用 `confCompute()` 小数(`1.00` 保留完整格式)。这页是全局分布图,不要展开逐格解释;细讲仍用前一页 5 个算子问题子集。**矩阵色阶固定为 2 红 / 3 橙 / 4 绿 / 5 蓝**,综合小数也按同一档位映射;不要只给综合列做层次。全量矩阵任务列要宽、栈列要窄,任务名尽量不换行;**场景放在任务左侧独立纵列**(选型转换/算子开发/训练迁移/环境容器/算子排障/训练排障/推理部署/性能优化/调试迁移),不要再用穿插分组行;指标表头是两层:官方触达 / 官方质量 / 生态支撑 / 成本与产出 → 具体指标。术语一律用**第三方**,不要使用旧称。
  ...
  - **时序图交互全套(`seq-d3.html`)**:① **两级聚光**——聚光态下悬浮**组内**某条线 → 它全亮、同组其他线 0.32 次要黯淡、非组 0.1;**只响应当前组内的线**。② **模态点击钉住**——点组内线钉住聚焦(再点取消/点别条切换),点空白或线外任意处取消(click 挂 `document`);`curFocus()=hoverMsg||pinnedMsg`,`go()` 切关系时重置。③ 每条线叠 **18px 透明热区线 + `cursor:pointer`**(细 SVG 线热区只有线宽、点不到,只有线上文字能点 → 加宽)。④ **左↔右联动**:`notifyHover()` postMessage `{__seqHover:curFocus()}`,父页按 `e.source` 定位 iframe→section;矩阵/问题页按 msg 里字母 D/M/X/O/L 压暗非匹配 `[data-letter]`;**开发者方法论页**(`#hstep0` 判定)按 **分支/决策 id→7阶段** 映射 `{b0:0,b1:3,bSoc:2,dSoc:2,b2:2,dVer:2,bTil:3,dTil:3,bAcc:5,dAcc:5}` 调 `setHStep`(⚠ Gitee 那条分支 id 是 `b2` 不是 `bVer`;决策块 dSoc/dVer/dTil/dAcc 也要入表)。⑤ 生命线高亮加粗变色(`data-col` 存色);第三方社区即使在组也半透(次要部分)。⑥ 两图左上角灰色图名(embedfix 注入 `.embed-title`)。⑦ **决策点(黄菱形)标题放菱形左上方**(`renderV` 里 `kind==="decision"` 的 `d.t` 标签 `text-anchor:"end"、x:-20`——用户嫌右边被箭头挤不好看;原作者本怕压「主导方」列才放右,实测放左可读)。⚠ **`seq-d3.html` 已和源 `../opknow/20_human_ai_journey.html` 大幅分叉**(生命线顺序重排、术语改名 第三方→第三方平台/官方→社区、决策标签左置、embedfix)——**别再当干净 cp,re-cp 会丢这些手改**;改时序图直接改 `seq-d3.html`。
  - **全屏时序图页 + 放大查看(2026-06-30)**:用户选方案 B,在原第 4 页(AI 执行架构)前新增一页 **`04/ 时序图 · 全屏`**,只放 `seq-d3.html?full=1` 的全屏时序图,让投影时先看完整关系。`seq-d3.html` 通过 `IS_FULL_SEQ` 判断 `?full=1`:只在 full 模式拉宽生命线间距(`Wv=1440`)、略增行距、主时序文字 `seqFont()` 放大;普通嵌入版保持原尺寸。`index.html` 给所有 `seq-d3.html` iframe 所在卡片自动加右上角 `.seq-zoom-btn`,点击打开 `.seq-modal` 全屏弹层,原有 `?hi=1/2` 会保留并追加 `full=1`。从新增全屏页翻到后面的 AI 执行架构页时,`triggerSeqMagic()` 会创建 `.magic-seq-ghost` 临时 iframe,按源图区域 → 下一页左图卡(`data-magic-target`)做 0.72s 缩放移动,模拟 Keynote Magic Move。⚠ 本地验证若用无头 Chrome/浏览器插件访问 `file://` 可能被系统策略拒绝,至少要做内联脚本语法抽取检查 + 页码/DOM 结构检查。
  - **🔴 对标术语铁律(用户反复强调)**:**CUDA ↔ CANN(技术栈对栈)、英伟达 ↔ 昇腾(品牌对品牌)**——**严禁「CUDA vs 昇腾」这种栈对品牌的错配**。AI 侧(矩阵/问法/方法论/指标)统一写成 **NVIDIA 侧「CUDA / 英伟达」 vs 昇腾侧「CANN / 昇腾」**(组合式,栈+品牌一起);开发者侧(基础体验)比品牌时用 **昇腾 vs NVIDIA**。已修 #6/#7 标题、#6 bridge、#7 问法两栏、#8 理论依据卡里的错配。新写文案务必照此对标。
  ...
  - ⚠🔴 **旅程页(#27/#29)宽度与裁剪处理(2026-07-01 修正)**:① 非 tponly 的 `?sec=` 嵌入沿用主报告 `.wrap{max-width:1180px;margin:0 auto}`——文字类楼层(j1–j5/actions/dept)保持限宽利于阅读,但 journey(7 列 grid)必须 `body.embed.sec-journey .wrap{max-width:none}` 解开限宽;② 之前为保宽排除了 journey 缩放,在当前 deck 右栏高度下会造成上下裁剪;现做法是**先压缩 journey 内部垂直节奏**(`jrn-*` 行高/内边距/情绪区高度),再允许 `fit()` 按高度轻缩,这样不牺牲横向宽度也不裁顶/裁底;③ deck 左侧说明栏保持 `flex:0 0 17%` 让 grid 更宽。**记住:嵌入楼层"变小/变窄/被裁"先查 `fit`/`scale` 与 `.sec-journey` 局部样式。**
  - **杂项 UI(2026-06-28)**:① **摩擦明细浮层**=`<details>` **点击**开合(`summary{cursor:pointer}`)+ `.djfi[open] .dj-pop{position:absolute;top:100%}` 浮层——浮在内容上、不挤压布局、不触发 `fit` 缩放(别做成 hover-only、别去掉点击,用户要点击展开)。② **#10–15 指标卡 `.mcard`**:去 `border-left` 左描边、改透白玻璃卡(对齐 `.card` 公式)。③ **#17 七个问题卡**:每张右上角加**淡水印 Lucide 线性图标**(`.p7-ic`,`opacity:.06`、`z-index:0` 压文字下层,`.p7>*{z-index:1}`)做配图,缓解纯文字。④ **总览页(按 O)悬浮出页码**:`#deck{counter-reset:slideno}`+`.slide{counter-increment:slideno}`+`.slide:hover::after{content:counter(slideno)}`。
  ...
  - **🔴 加载优化(2026-06-28,用户嫌慢 + 先闪原始页)**:两件事。① **iframe 懒加载**:deck 有 **60 个 iframe**,一次性并发加载极慢;把 `index.html` 里所有 `<iframe src=...>` 改成 `data-src`,加 `lazyFrames(center)`(只给当前页及相邻 ±1 的 `iframe[data-src]` 注入 `src` 后 `removeAttribute('data-src')`),在 `updateCurrent(idx)` 里调用(初始化 `updateCurrent(start)` 也会触发)。相邻预载→顺序翻页时下一页已就绪。⚠ 总览模式未访问过的页 iframe 是空白(但 deck 标题在外层、够导航);新增 iframe 记得用 `data-src`。② **no-flash(先隐藏再淡入)**:embedfix 跑在 `setInterval` 里,所以原始报告先整页渲染、几百 ms 后才被裁剪——中间闪原始页。**每个 embed(ar-embed/ux19-embed/m18-embed/seq-d3/arch-d3)`<head>` 末尾**加 `<style>html.embed-wait body{opacity:0}body{transition:opacity .14s}</style>` + 一段把 `embed-wait` 类**同步**加到 `<html>`(body 绘制前)的 `<script>`(带嵌参的报告类条件加;seq/arch 这种纯嵌入文件无条件加),并在各自 embedfix **首次成功**处 `document.documentElement.classList.remove('embed-wait')` 揭显 + `setTimeout(reveal,3000)` 兜底。轮询顺带从 80/90ms 提到 30/40ms、加 `DOMContentLoaded` 即试一次,揭显更快。⚠ 揭显**必须**挂在 embedfix 成功点,只靠 3s 兜底会黑屏 3 秒(比闪烁更糟)。
  - **AI 指标定义重做成 5 页(2026-06,替换原 #8 seq-d3+mxg)**:用户要去掉时序图、放 `../opknow/18_metric_definition.html` 的 §s3(指标怎么形成)+ §s4(逐项定义),并加一页「理论权威依据」防质疑。① **理论依据**(自写 `.auth` 6 卡:对照实验 / 信息觅食价值链 / findability·retrievability / parametric knowledge / noisy-OR / score_metrics 可复现)——只引真实可核查经典文献/论文;标题直接陈述结论。② **§s3 怎么形成**(iframe `m18-embed.html?sec=s3`)③④⑤ **§s4 逐项定义按组拆 3 页**。**2026-06-30 标签/配色修正**:页 #9/`m18-embed.html?sec=s3` 及后续指标卡统一遵循 `report-ppt-skill` 灰底规范。**Codex 正式 skill 注册**:`report-ppt-skill` 已从 `/Users/hsin/.claude/skills/report-ppt-skill` 复制到 `/Users/hsin/.codex/skills/report-ppt-skill`,后续新会话应按正式 Codex skill 触发;本项目仍可直接读该 skill 的 `references/type-and-color.md`/`checklist.md`。
  - **结论 2 页(20 核心结论 + 21 设计点链路,2026-06 新增)**:用户要「突出之前做的 5 个设计点,且体现 **分析→结论→设计点** 的链路」。① **20 核心结论**=3 张 `.cc` 卡(AI侧/开发者侧/合),各带证据 chips(矩阵.41 / 架构图杜撰 / 配环境−11 / Toolkit−66 / 上手L2反超…)。② **21 设计点·链路**=`.chain` 网格,每行 [号+人/AI 徽标] + 三栏 **分析依据 → 结论 → 设计点**(设计点格 AI 紫 / 人 蓝)。5 个设计点:①智能助手把碎片化信息总结为场景化结构化答案 ②文档=即取即用/可对话/可运行的智能触点 ③沉浸交互式学习 ④基于意图的学习路径推荐 ⑤社区知识 skill 化复用给外部大模型(=①②④对 AI 侧的收口)。归属:①②⑤=AI侧分析推出、③④=开发者侧旅程推出。证据全部锚回 deck 前面页,不是凭空列设计点。⚠ **字号走 ramp 上档**:结论卡标题 `--fs-h2`、正文 `--fs-body`、链路格 `--fs-body`、徽标 ~30px,`.ccg`/cell 用 `flex:1` 撑满竖向空白——**别用 xs/sm 那两档**(初版用低了、字太小、页面留白多,已上调)。
  - **术语口径(2026-06-30)**:面向观众的综合汇报里尽量不用 `SDK`,统一改为 **Toolkit**(如「Toolkit 下载」「配环境·拿 Toolkit」);泛指移动侧时写「移动端开发包」,避免生硬的「移动 Toolkit」。
  - **结论行动页 tab / rgoal(2026-06-30)**:顶部板块 tab 已移到右上角 `.head-r.board-tabs`,替代原来的「行动页/板块」小字;只保留 tab,不放「负责的板块」说明。点击跳转靠 `.bk-strip[data-group="exp|doc|dev"]` 分组,不要再依赖说明文案。`div.rgoal` 右下角语义图标按 `data-board` 的 `1.1/1.2/.../3.2` 前缀匹配,别用旧 `01/02` 前缀,否则 mask 失效会露成方块。图标用 Lucide 风格的简洁线性路径,不要手写复杂交叉图标,水印放大后线条穿插会很明显。`rgoal` 左栏保持 30% 宽,不要为解决换行加宽挤压右侧。⚠ `rgoal` 是 flex column,正文必须包在 `.rgoal-t` 里;否则裸文本和 `<b>` 会各自成为 flex item,导致加粗词独占一整行。重点短语仍用黑色加粗 + 轻底色,`b` 保持普通 inline,不要做成 tag/inline-block/nowrap。
  - **基础体验结论页 #60–64(2026-07-01)**:外层标题和 `JUDGES` 文案都改成更短、更结论导向的说法。嵌入页样式用 report-ppt-skill 的「黑卡主结论 + 对比点列」:`.lead` 是黑底主结论卡,下面每段做带圆点的对比说明,不要再做一排浅色证据卡;加粗词用黑色加粗 + 衬色底,黑卡内加粗用白字衬色。不要再写大段解释;每页只突出一个主结论。#65 三角色总览颜色必须和后续角色页对应:体验设计=紫 `#5b5bd6`,资料内容=青 `#1788a8`,开发=蓝 `#2f6fed`;每张卡先放黑色 `.cc-focus` 重点条,再放职责与指标,避免文字平均用力。
  ...
  - **⚠ 放大字号必分两类(用户反复强调)**:① **「写在材料上的字」**(直接铺在页面底上的内容/标签:阶段条 `.lc-stages`、任务卡 `.op5-i b/small`、开发者侧 7 步 `.op7-i b`、触点分析 `.hs-ana`、问题对内容 `.qx-intent/.qx-c`、结论卡正文…)→ **该放大**(普遍 xs/sm→sm/body)。② **「组件/图表里的字」**(受组件尺寸约束:**矩阵 `.hm-*`**、DJFI 网格 `.djg`、指标网格 `.mxg`、时序图/架构图 iframe 内部 SVG、各种徽章/页码/轴标签)→ **不动**,放大就挤破/溢出。**「整体调大」≠ 全局 ramp×N**——要逐类区分,只放大第①类。(把硬编码 px 收敛到 xs 不会变大,因为 xs=9–11≈原值;真要大必须升一档到 sm/body。)**已按 skill「角色→档位」映射系统性过一遍**(正文/说明/场景→body):副标题 `.subttl`、信任落差三卡正文 `.dh-d`、总览映射卡 `.ovm-d`、指标页总结句 `.mxline` 等全升到 `--fs-body`;`.seq-note` xs→sm。**别再点哪改哪——新增页直接照角色映射给档**。
  - **第5页「问题对」**:每任务补「**同一意图**」横幅(取自 opknow 18 §2)+ 名称/意图/两问装进**半透明白卡 `.qtask`**(无描边);并去掉「CUDA 不放水、昇腾不刁难」。
  - **⚠ 收尾铁律(本人反复忘)**:每轮改完**必须**同步本文件 + commit + push,不要攒到最后;用户多次因没更新 AGENTS.md 而不满。arch-d3 的改动**两份**(本仓 `reports/synthesis/arch-d3.html` + 源 `../opknow/20_human_ai_journey.html`)都要改并各自 commit/push。
  - **收尾铁律(2026-06-30 用户明确要求)**:每次改完本项目后,必须同步更新 `AGENTS.md`(本文件)记录最新状态/坑点,并主动 `git add` + `git commit` + `git push`。如果权限/网络导致无法 push,要在最终回复明确说明阻塞原因和已完成到哪一步。
  - **🟢 2026-06-27 改版(防裁剪 + 分页 + 滚动 + 小标题规整)**:
    - **m18 §s4 逐项定义从 3 组拆成 6 组(防裁剪)**:原 `?sec=s4&grp=touch|qual|costout` 三页里 ⑪ 综合那页(costout)最长被裁。`m18-embed.html` 给 11 个 `.mcard` 各加 `data-pg="a..f"`(a=①②官方触达 / b=⑤⑦第三方&自带 / c=③④官方质量 / d=⑥第三方质量 / e=⑧⑨⑩成本产出 / f=⑪综合),embedfix JS 支持 `grp=a..f` 按 `data-pg` 过滤(旧 touch/qual/costout 仍兼容)。deck 对应 6 页 `?sec=s4&grp=a..f`。**并删了 §s3 的「第三步/第四步」**(过三道关筛 + v1–v5.1 演化时间线)——s3 页太长。
    ...
    - **⚠⚠ 两个致命 bug(2026-06-27 三轮,用户暴怒「还有滚动条」「每页非得放昇腾vs英伟达」)**:
      - **① hero 漏露**:embed 模式隐藏其它楼层用的是 `ov.querySelectorAll('section.floor')`——但 **`#hero` 是 `<header class="hero floor">`(header 标签、不是 section)**,选不中、于是「昇腾vs英伟达 均分 77/83、Toolkit下载、取证方法」那整块 hero 跟着 #method 一起露出来(还顶高了页面→滚动条)。**改用 `.floor`**(连 header 一起选)只留目标楼层。**记牢:隐藏楼层一律用 `.floor`、永远别加 `section` 限定**。
    ...
    - **小标题(`.subttl`)规整成「章节 · 「本页内容」」**:原各页 eyebrow 杂(有的写英文 `Per-item definitions`、有的写来源 `opknow 18 §4`)。统一为 `章节名 · 「本页一词概括」`(如 `AI 侧 · 指标体系 · 「综合置信度 ⑪」`、`开发者侧 · 基础体验 · 「责任归属 P1」`)。批量替换用 python 按内容匹配(终端中文显示乱码但读写正常,UTF-8 无 BOM)。
    - **visual.html 顶部 tab 加 hash 定位**:`#overview/#op/#train/#comm/#synth` 直达对应 tab(`showTab` 写 `replaceState`、加载与 `hashchange` 读 hash)。
  - **🟢 结论已重做成「分角色行动方案」(2026-06-30,替换旧 行动建议+责任归属 iframe 页)**:用户要求结论部分**详细到能推动改进、讲完别让人懵**。现结构 = **三角色总览 1 页 + 3 个角色**(`体验设计`紫 / `资料内容`青 / `开发`蓝,把旧 6 部门并成 3)× **8 个行动页**:体验设计=官网与落地/下载与账号/学习与上手/**文档体验**;资料=**文档内容与版本**/社区与内容;开发=机器可读出口/工具与工程。**文档相关板块不能共用一页**:体验设计负责界面、路径、查找、阅读、局部 FAQ、分诊与跳转;资料内容负责正文、版本口径、错误码、依赖矩阵、样例质量。`智能化触点`不是独立板块,只是搜索/FAQ/助手/分诊等横向能力,已拆进文档体验、社区与内容、机器可读出口等真实板块。旧 `ar-embed.html?sec=actions`、`ar-embed.html?sec=dept&pg=1/2` 三页已从 deck 删除,但历史 embed 能力可保留。
    - **版式方案 B(用户拍板,后续微调)**:每个板块页是「左侧决策摘要 + 右侧优先级行动板」,不是平均清单。`.rolewrap` 两列:左 30% `.rgoal` 玻璃决策卡,右 70% `.ameasures` 行动板。`.rgoal` 顶部用 `data-board` 显示 `1.1 · 官网与落地` / `1.4 · 文档体验` / `2.1 · 文档内容与版本` 这类**角色序号 + 角色内板块序号 + 板块名**,不要用全局流水号;**不要写“本页拍板”**。`.rgoal` 不要左描边,右下角用 `--rg-icon` 的淡线性语义图形(入口/下载/学习/搜索/文档/社区/代码/工具),不要再用纯圆。**外层只按优先级分组**:`.ahead.p0/p1/p2` 文案只显示 `P0/P1/P2`,不要横线、不要“先拍板/接着做/后续补齐”等口语;`.rmi:has(.p0)` 横跨两列做大卡,P1/P2 做次级卡。**对象维度只能做卡内小标签**:`.who2.hu`=`开发者`、`.who2.co`=`开发者 + AI`、`.who2.ai`=`AI`,不要再把“给开发者 / 共通 / 给 AI”放到外层标题。为让 CSS 根据角色换色,每个 `.rolewrap` 内放一个隐藏 `.role-chip exp/doc/dev`;页头的 `.role-chip` 仍负责可见角色标签。页头 `.bk-strip` 也跟角色走色:`.head-l:has(.role-chip.exp/doc/dev)` 设置 `--tab-role/--tab-soft`,当前 `.bk.on` 用角色实色,其它 `.bk` 用角色淡底。
    - **结论行动页顶部 tab 可点击(2026-06-30)**:`.bk-strip .bk` 不是装饰,已由 `installBoardTabs()` 自动建立同一组内「板块名 → slide index」映射。映射按 `.bkl` 分组、读取每页 `.bk.on` 的当前板块名,因此插页/页码变化不需要手写目标页码。点击或键盘 Enter/Space 调 `go(target)`,URL hash 会随 `updateCurrent()` 自动同步。新增行动板块页时只要保证同组 `.bkl` 文案一致、当前页对应 tab 带 `.on`,跳转就会自动生效。
    - **内容覆盖口径**:8 页动作已覆盖触点 `rec` 主线(官网入口/导航/选型/规格/下载/登录/安装/学习/示例/ModelZoo/工具/论坛/FAQ/博客/语言)、AI 侧七个问题(版本混乱、自带知识偏薄、第三方少而同质、SPA 抓不到、抓得到不够用、深水区塌方、检索成本尖刺)与内容可用性结论 8 发力点(llms.txt/Markdown/SSR/页面组织/错误码/第三方生态/语料供给等)。设计点 ①–⑤ 已融入具体措施,不再单列 demo。
    - **验证限制**:本地浏览器工具因安全策略拒绝直接访问 `file://.../reports/synthesis/index.html`,未能自动截图新结论页;已做静态校验(脚本语法、页数、旧 iframe 清除、术语残留)。若肉眼打开后某页拥挤,优先微调 `.rolewrap` 列宽、`.rmi` padding/gap、P0 卡数量与 `.rmi-why` 字数。
...
- **事实与口径**:平台、仓库、搜索结果和截图必须经真实来源核验；不得把“官方来源不计第三方”写成“未找到”。模拟内容须明确为示意，不能替代研究事实。
- **保留用户文案**:编辑前读取文件最新状态；用户直接改写的文案和未点名内容不得覆盖、删改或随批量重写丢失。
- **材料语言**:不把讨论过程、辩解式说明、内部代号或未经验证的假设写入交付材料；正文采用简洁、书面、可复核的表达。
- **局部改动与视觉核验**:用户点名页面或组件时仅改对应范围，并检查关系表达、对齐、重叠、裁切和文字溢出；新增图片、图标本地化，保证离线浏览与 PDF 导出稳定。
