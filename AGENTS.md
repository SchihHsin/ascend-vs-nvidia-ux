...
- **⚠ 当前状态(2026-06-24,覆盖上面若干旧描述)**:
  ...
  - **矩阵后新增全量矩阵页(2026-06-30,2026-07-01 调整)**:算子子集矩阵后加一页 `opknow 全量矩阵`,展示 26 个任务 × 11 指标,每任务 CUDA/CANN 两行,综合列同样用 `confCompute()` 小数(`1.00` 保留完整格式)。这页是全局分布图,不要展开逐格解释;细讲仍用前一页 5 个算子问题子集。**矩阵色阶固定为 2 红 / 3 橙 / 4 绿 / 5 蓝**,综合小数也按同一档位映射;不要只给综合列做层次。全量矩阵任务列要宽、栈列要窄,任务名尽量不换行;任务按场景插分组行(模型转换与选型/自定义算子深层任务/训练与迁移配置/环境与容器/算子排障与集成/训练排障/推理部署/性能优化/调试与迁移)。术语一律用**第三方**,不要使用旧称。
  ...
  - ⚠🔴 **旅程页(#27)宽度老调不好的根因 = 两件事叠加(2026-06-28 终于定位)**:① 非 tponly 的 `?sec=` 嵌入沿用主报告 `.wrap{max-width:1180px;margin:0 auto}`——文字类楼层(j1–j5/actions/dept)保持限宽利于阅读,但 journey(7 列 grid)被限宽居中→两侧空白;② **更隐蔽**:embedfix 末尾的「缩放适配」`if(fh>vh-2)scale(...)` 用 `transform-origin:top center`,journey 比 iframe 高时被**整体缩小+水平居中**→更窄、左右都空(视口越矮缩得越狠)。**修法**:① apply 里 `body.classList.add('sec-'+id)`,`body.embed.sec-journey .wrap{max-width:none}` 解开限宽(别全局解,文字楼层会被撑太宽);② **缩放那行加 `id!=='journey'` 排除**——journey 是宽幅 grid,宁可铺满宽度(矮了底部留白),也别为塞高度而缩小变窄;③ deck 左侧说明栏 `flex:0 0 22%`→`17%` 让 grid 更宽。**记住:嵌入楼层"变小/变窄"先查这行 `fit`/`scale` 是不是把它整体缩了。**
  ...
  - **基础体验结论页 #60–64(2026-07-01)**:外层标题和 `JUDGES` 文案都改成更短、更结论导向的说法。嵌入页样式用「主结论卡 + 证据卡」: `.lead` 是整体底色主卡,下面每段是浅底证据卡,加粗词用黑色加粗 + 衬色底。不要再写大段解释;每页只突出一个主结论。#65 三角色总览颜色必须和后续角色页对应:体验设计=紫 `#5b5bd6`,资料内容=青 `#1788a8`,开发=蓝 `#2f6fed`。
