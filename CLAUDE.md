...
- **页3「逐步讲解」聚光灯动效**(用户要的「弱化其余/突出当前/逐个讲」):在 `seq-d3.html` 里做(比 iframe 外层干净)。`renderV()` 末尾导出 `window.__seqGeo`(vTop/dyV/Hv/Wv + 各 `PHASE` 的 top/bot 像素带);末尾 embedfix 脚本建右上角控制条 `#seqctrl`(‹ / ▶逐步讲解 / ›)+ `applySpot()`:按 `PHASE`(7 段:P1立项/P2-P6=D·M·X·O·L 五检索/P7交付)逐步,用两块半透白 `#spotmask` rect 压暗当前阶段横带以外的区域 + 左侧紫条高亮当前段;step 0=全亮,循环 0→1..7→0。重绘(滑块)后 renderV 回调 `__seqApplySpot()` 回填。**没绑方向键**(避免和 deck 翻页冲突),只点按钮。关联说明:**AI↔昇腾/二手=AI 可用性矩阵打分(opknow)**、**人↔昇腾社区=触点体验(ascend-vs-nvidia-ux)**。
