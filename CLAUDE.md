...
- **⚠ 当前状态(2026-06-24,覆盖上面若干旧描述)**:
  ...
  - **矩阵后新增全量矩阵页(2026-06-30)**:算子子集矩阵后加一页 `opknow 全量矩阵`,展示 26 个任务 × 11 指标,每任务 CUDA/CANN 两行,综合列同样用 `confCompute()` 小数(`1.00` 保留完整格式)。这页是全局分布图,不要展开逐格解释;细讲仍用前一页 5 个算子问题子集。
  ...
  - **🔴 单触点页 25 页 = `ar-embed.html?tp=ID`(2026-06-30 改为证据页)**:用户反馈 #33–57 字太多、图太小后,单触点页改成**证据页**:顶部一行两卡=「算子开发者视角」+「摩擦指数」;中间 **昇腾 / NVIDIA 两张大图并列**;底部只保留 1–2 条简短差异分析。**不再显示「昇腾该怎么做」**,行动建议交给后面的结论行动页统一讲。实现口径:`.tpsolo` 三行网格 `auto 1fr auto`,`.tpleft{display:contents}` 让 `.djfi` 和 `.tpcard-body` 进入主网格;`.shotcap` 隐藏;`.concl li:nth-child(n+3)` 隐藏;`.rec{display:none}`。摩擦明细仍可点击展开浮层,但不占布局。
  - **AI 侧问题/发力点 8 页(2026-06-30,矩阵后)**:把 `../opknow/19_findings_ux_synthesis.html` 的「七个问题 / 供给侧」搬进来。**①七个问题(§1)= 纯文本丑、用户要求重排**:手搓 `.pg7`(4×2 网格,7 问题卡 + 1 深色收口格,①② 标红「结构性」)。**②供给侧 §3 八卡 = iframe 嵌原卡但做投影精简**:`reports/synthesis/ux19-embed.html?card=1..8` 隐卡内 `h3`,左侧只保留前两条重点 + why,隐藏长的 `.nvref`,右侧 demo 放大。⚠ 有 demo/精调的仍嵌,但汇报页要少字、书面、重点明确。
