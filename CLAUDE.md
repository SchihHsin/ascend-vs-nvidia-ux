...
- **页3「按关系讲解」高亮动效**(用户要的不是横向时间带、而是**竖向按生命线对**:讲矩阵评分=高亮 AI↔昇腾/二手 的所有箭头、其余压暗;讲技术体验=高亮 开发者 ↔ 昇腾社区 的线)。在 `seq-d3.html` 里做:`renderV()` 给每条消息元素打 `data-pair`(`[frm,to].sort().join('-')`,如 `ai-off`/`ai-sec`/`hu-off`/`hu-sec`/`ai-hu`)、每条生命线打 `data-lane`(BRANCH 人独立分支线 pair=`hu-`+目标)。末尾 embedfix 脚本建右上控制条 `#seqctrl`(‹ / ▶按关系讲解 / ›)+ `applyHi()`:遍历 `#journey` 直接子元素——命中 `GROUPS[step].pairs` 的 `data-pair`→opacity 1、命中涉及 lane 的 `data-lane`→1、其余(决策/自身节点/阶段卡/无标记)→0.1~0.12;step 0=全亮(清 inline opacity)。`GROUPS`=[AI↔昇腾/二手·矩阵评分, 开发者 ↔ 昇腾社区·技术体验](2 组),循环 0→1→2→0。重绘(滑块)后 renderV 回调 `__seqApplyHi()` 回填。**没绑方向键**(避免和 deck 翻页冲突),只点按钮;蒙层不挡 hover(改的是 opacity、不加遮罩,`pointer-events` 不变)。
...
- **⚠ 当前状态(2026-06-24,覆盖上面若干旧描述)**:
  ...
  - **🔴 单触点页 25 页 = `ar-embed.html?tp=ID`(2026-06-27,一页一触点)**:用户最终要的开发者侧触点形态=**一页放一个完整触点卡**(头部 + 算子开发者视角流程 + 昇腾/NVIDIA 两张对比图 + 摩擦指数行 + 差异分析 + 昇腾该怎么做),参照原报告触点卡。只放**算子开发**相关触点(去训练),按 deck 顺序 `['1.1','1.1b','1.2','2.1','2.3','2.4','2.2','3.2','3.3','5.4','4.1','4.2','4.3','4.4','3.1','3.4','5.1','5.3','6.1','6.2','6.3','5.2','6.4','1.3','1.4']` 共 25 页,每页 `<iframe src="ar-embed.html?tp=ID">`,deck 头给标题(`{id} · {name}` + 副标 `开发者侧 · 基础体验 · 触点详情　·　昇腾 X / NVIDIA Y(差值 Z)`)。embedfix `?tp` 分支:`body` 加 `.tponly`,沿 `#tp-ID` 祖先链隐藏所有旁支兄弟只留这一条 DOM、给目标卡加 `.tpsolo`、隐 `.tpcard-hd`(deck 头已给名)+ `.shotcap`(和差异分析重复、留着会撑高被裁)、`fit()` 缩放到一屏(ResizeObserver + img onload 重 fit)。**排版=左右版**(用户定:宽而短的一屏,竖排会让图小/底部裁切):`.tpcard.tpsolo` 设 `display:grid;grid-template-columns:64% 36%`——`.pov`(视角)`grid-column:1/3` 占顶部整宽;**JS 在 apply 里把 `.djfi`(摩擦分,置顶)+ `.tpcard-body`(两图)包进新建的 `.tpleft`**(flex 列、gap 12px、`grid-column:1`),右列 `.verdict`(差异分析+建议)`grid-column:2`;**视角/摩擦/图/差异分析/建议全是同款卡片**(白底+1px `--g3`+12px 圆角),卡间距统一 12px。⚠🔴 **左栏两卡之间忽大忽小的空隙 = grid 行被右侧高卡撑开**(原本 `.tpcard-body` 在 row2、`.djfi` 在 row3,verdict 跨 row2/4;verdict 比左栏高时,多出的高度被等分进 row2/row3,图卡和摩擦卡就被推开,图越"长条/矮"越明显)——**修法=用 `.tpleft` 把左栏两卡裹成一个 flex 列**,它俩永远紧挨、固定 12px,空隙只留到左栏底部。**分数(摩擦)放图上面**(用户定:和右栏差异分析标题对齐成"小结排"、且原报告分数本就在顶部)。图 `width:100%`(宽度驱动、别用 `width:auto`);**长条形图(很宽很矮,JS 测 `naturalHeight/naturalWidth<0.4`)自动加 `.imgstack` 改上下排**(各占整栏宽放大),普通比例保持左右。⚠ **摩擦明细(`<details>`)别内联展开**——会撑高卡片触发 `fit()` 把整个 iframe 缩小、很难读;tponly 下把明细内容包进 `.dj-pop`、`hover .djfi` 时**绝对定位浮层弹出**(`position:absolute;top:100%`,不占布局高度、不缩放;`display:block!important` 覆盖 details 关闭时的 `display:none`,visibility 控制显隐)。rec(昇腾该怎么做)在 tponly 下改 grid:`rec-tag`+`rec-dept` 同一行(标签左/主责右)、`rec-txt` 另起一行铺满(窄右栏三者并排会把 txt 挤成一条)。⚠ **绿卡(verdict)右内边距"特别小"= `.concl li` 是 `padding:7px 0 7px 26px`(左 26px 给 ◆ 项目符号、右 0)**,宽卡看不出、窄右栏里文字就贴右边、左右失衡;修法 `body.embed.tponly .tpcard .concl li{padding-right:22px}` + `.rec{padding:13px 18px}` 补右内边距(scope tponly、不动主报告)。⚠🔴 **右侧"被裁、圆角被切"的真因 = grid 溢出被 iframe 裁,不是 padding/margin**:`grid-template-columns:64% 36%` + `column-gap:18px` = **100%+18px**,超出 iframe 宽 18px,右边绿卡溢出部分被 iframe 边界裁掉(圆角消失)。**修法:百分比改 `fr` 单位 `64fr 36fr`**(fr 自动扣掉 gap、不溢出)+ `.tpsolo{overflow:visible}`。(先后误判成 `.s-gray` 的 `4vw` padding、`.concl li` 右内边距、`.body-area` 负边距,都没解决——记住:卡片右边/圆角被切先查 grid 轨道和是不是百分比+gap 溢出。)⚠ **副标题分数用 RESCORE/MVAL 后的 final 值**、不是 TP 原始 `a/nv`(原始的会比卡内分数差几分,不一致)。 ⚠🔴 **`.tpgrid` 是 2 列网格**——`#tp-ID` 作为网格项只占**一列(半幅)且靠左**;`tponly` 必须 `.tpgrid{display:block!important}` 才铺满整宽。 ⚠🔴 **grid 的 `display:grid!important` 只能 scope 到 `.tpsolo`、不能落到广谱 `.tpcard`**——否则 `!important` 顶掉 JS 给兄弟卡设的 inline `display:none`,被隐藏的相邻触点卡(如 1.1b)会一起冒出来(同 ux19 `.ux-show` 坑,2026-06-27 又踩)。
  ...
  - **AI 指标定义重做成 5 页(2026-06,替换原 #8 seq-d3+mxg)**:用户要去掉时序图、放 `../opknow/18_metric_definition.html` 的 §s3(指标怎么形成)+ §s4(逐项定义),并加一页「理论权威依据」防质疑。① **理论依据**(自写 `.auth` 6 卡:对照实验 / 信息觅食价值链 / findability·retrievability / parametric knowledge / noisy-OR / score_metrics 可复现)——**每张卡底部配真实权威出处 `.au-src`**(Fisher《实验设计》1935 / Pirolli&Card 信息觅食 1999 / Morville《Ambient Findability》+Azzopardi retrievability / Lewis RAG 2020+Petroni 参数化知识 / Pearl noisy-OR 1988 / Pineau ML 可复现性):用户要求**引经据典、别只自己解释**,但**只引真实可核查的经典文献/论文,绝不编造**;标题**别用辩解式**「并非主观臆定」、直接陈述结论。**每卡内容=「依据→做法→指标」三段链**(理论说什么→我们怎么操作→得出哪项指标;`.au-step` 用 `display:block`+`.au-k` `inline-block`,**别用 flex/grid**——会把每个 `<b>` 当成独立 flex item 导致断行/空隙)。每卡右上角配 **Lucide 线性 icon**(内联 SVG、git-compare/link/search/database/git-merge/refresh-cw),做成**大号低透明度水印**(`opacity:.06`、放大铺右上、被卡边 `overflow:hidden` 裁切)——给右侧加视觉重量、平衡左侧彩色标签列(用户:左重右轻;`.au>*{z-index:1}`+水印 `z-index:0` 压到文字下层)。卡片用**透白玻璃卡**(对齐 `.card` 公式:`rgba(255,255,255,.55)`+`backdrop-filter:blur(15px) saturate(1.4)`+白边+柔阴影+inset 高光)。⚠ **配色铁律:装饰元素(au-ic/出处)中性灰,颜色只留给有含义的功能标签——依据=琥珀 `#b45309`/做法=紫 accent/指标=绿 `#1f7a4d`**(三步各一色、成色码);**au-tag(类别)用中性灰药丸**(唯一中性、和三个功能色标签区分)。⚠ 别让 au-tag 和「依据」长一样(都灰药丸时分不清,用户打回)——靠"依据加色 + tag 保持灰"区分,**别**把 au-tag 改成下划线眉标(试过,用户嫌丑)。否则 6×(tag+icon)全 accent 紫=满页二十多处紫、没主次、"AI 味重"(用户打回)。② **§s3 怎么形成**(iframe `m18-embed.html?sec=s3`)③④⑤ **§s4 逐项定义按组拆 3 页**(`?sec=s4&grp=touch|qual|costout`,11 个 `.mcard` 按 touch/qual/cost+out+sum 分组)。`m18-embed.html`=`cp` opknow18 + embedfix(opknow18 **无 section 包裹**,h2 平铺在 `.wrap`,故按「从 `#sX` 显示到下一个 `#s(X+1)` 之前」裁;§s4 组页只显示对应 `.mcard.<grp>`、隐 h2 与 intro note)。改 opknow18 后要重 cp+注入。**2026-06-30 标签/配色修正**:页 #9/`m18-embed.html?sec=s3` 及后续指标卡统一遵循 `report-ppt-skill` 灰底规范:背景用 `#E9EBEE` + 透白卡;装饰/类别标签统一中性灰淡底 + 1px 边框;只有语义标签使用低饱和淡底(价值链的第 1 关=触达蓝紫、第 2 关=质量紫、摩擦=琥珀、结果=产出绿;指标卡同口径:触达蓝紫、质量紫、成本琥珀、产出绿、反向浅琥珀),禁止深色实底小药丸和满屏强色块。`report-ppt-skill` 当前在 `/Users/hsin/.claude/skills/report-ppt-skill`,Codex 可直接读取作为本地规范;若要自动注册成 Codex skill 再迁到 `~/.codex/skills/` 或插件目录,本项目迭代不必移动。
  ...
  - **🟢 结论已重做成「分角色行动方案」(2026-06-30,替换旧 行动建议+责任归属 iframe 页)**:用户要求结论部分**详细到能推动改进、讲完别让人懵**。现结构 = **三角色总览 1 页 + 3 个角色**(`体验设计`紫 / `资料内容`青 / `开发`蓝,把旧 6 部门并成 3)× **8 个行动页**:体验设计=官网与落地/下载与账号/学习与上手/**文档体验**;资料=**文档内容与版本**/社区与内容;开发=机器可读出口/工具与工程。**文档相关板块不能共用一页**:体验设计负责界面、路径、查找、阅读、局部 FAQ、分诊与跳转;资料内容负责正文、版本口径、错误码、依赖矩阵、样例质量。`智能化触点`不是独立板块,只是搜索/FAQ/助手/分诊等横向能力,已拆进文档体验、社区与内容、机器可读出口等真实板块。旧 `ar-embed.html?sec=actions`、`ar-embed.html?sec=dept&pg=1/2` 三页已从 deck 删除,但历史 embed 能力可保留。
    ...
    - **内容覆盖口径**:8 页动作已覆盖触点 `rec` 主线(官网入口/导航/选型/规格/下载/登录/安装/学习/示例/ModelZoo/工具/论坛/FAQ/博客/语言)、AI 侧七个问题(版本混乱、自带知识偏薄、第三方少而同质、SPA 抓不到、抓得到不够用、深水区塌方、检索成本尖刺)与供给侧 8 发力点(llms.txt/Markdown/SSR/页面组织/错误码/第三方生态/语料供给等)。设计点 ①–⑤ 已融入具体措施,不再单列 demo。
