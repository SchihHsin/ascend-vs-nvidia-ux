...
- **右栏随高亮联动**(用户要的「聚到 AI↔昇腾→右边出矩阵评分分析、聚到人侧→出 ascend ux 分析」):iframe 每次 `applyHi()` 用 **postMessage** `window.parent.postMessage({__seqStep:step},'*')` 通知父页(**不能直接 `window.parent.__seqOnStep` —— file:// 跨框同源策略会静默挡掉**);deck 第3页 `window.addEventListener('message',…)` → `__seqOnStep(n)` 切 `.seq-side` 里的 `.ss-pane[data-step=n]`(0=总览/1=矩阵评分过程+结论/2=技术体验过程+结论)。⚠ `.ss-pane` 自定义了 `display:flex`,会盖住 `hidden` 属性,必须补 `.ss-pane[hidden]{display:none}` 才能正确隐藏。
