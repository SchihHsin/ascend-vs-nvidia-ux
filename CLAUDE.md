...
- **⚠ 当前状态(2026-06-24,覆盖上面若干旧描述)**:
  ...
  - **🔴 单触点页 25 页 = `ar-embed.html?tp=ID`(2026-06-27,一页一触点)**:用户最终要的人侧触点形态=**一页放一个完整触点卡**(头部 + 算子开发者视角流程 + 昇腾/NVIDIA 两张对比图 + 摩擦指数行 + 差异分析 + 昇腾该怎么做),参照原报告触点卡。只放**算子开发**相关触点(去训练),按 deck 顺序 `['1.1','1.1b','1.2','2.1','2.3','2.4','2.2','3.2','3.3','5.4','4.1','4.2','4.3','4.4','3.1','3.4','5.1','5.3','6.1','6.2','6.3','5.2','6.4','1.3','1.4']` 共 25 页,每页 `<iframe src="ar-embed.html?tp=ID">`,deck 头给标题(`{id} · {name}` + 副标 `人侧 · 基础体验 · 触点详情　·　昇腾 X / NVIDIA Y(差值 Z)`)。embedfix `?tp` 分支:`body` 加 `.tponly`,沿 `#tp-ID` 祖先链隐藏所有旁支兄弟只留这一条 DOM、隐 `.tpcard-hd`(deck 头已给名)+ `.shotcap`(和差异分析重复、留着会撑高被裁)、`.shot/.mock` 限 `max-height:34vh`、`fit()` 缩放到一屏(ResizeObserver + img onload 重 fit)。⚠ **副标题分数用 RESCORE/MVAL 后的 final 值**、不是 TP 原始 `a/nv`(原始的会比卡内分数差几分,不一致)。 ⚠🔴 **`.tpgrid` 是 2 列网格**——`#tp-ID` 作为网格项只占**一列(半幅)且靠左**,看着"宽度很小";**`tponly` 必须 `.tpgrid{display:block!important}` 才铺满整宽**(2026-06-27 踩:以为是 `.tpcard` max-width 或缩放,其实是父 grid 分列)。
