...
- **⚠ 当前状态(2026-06-24,覆盖上面若干旧描述)**:
  ...
  - **🟢 2026-06-27 改版(防裁剪 + 分页 + 滚动 + 小标题规整)**:
    ...
    - **⚠⚠ 两个致命 bug(2026-06-27 三轮,用户暴怒「还有滚动条」「每页非得放昇腾vs英伟达」)**:
      ...
      - **② 分组分页仍会溢出→滚动**:把 7 个编号点塞成 2–3 页,单页还是太满出滚动条。**用户要求「一个编号点一页」**——`#method` 的 `一~七` 七段各打 `data-pgstart="2..7"`(一不打、默认 pg1;两个「七」算例+可复现性局限合并进 pg7),deck 对应 **DJFI 7 页**(`?sec=method&pg=1..7`,head-r `22b~22h`)。一段一页内容少、天然不溢出。**铁律:嵌长楼层=按它自己的编号小节一节一页,别图省页数硬塞**。
