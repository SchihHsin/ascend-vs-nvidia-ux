...
- **⚠ 当前状态(2026-06-24,覆盖上面若干旧描述)**:
  ...
  - **🔴 结论重做成「分角色行动方案」(2026-06,进行中,替换旧 行动建议+责任归属 iframe 页)**:用户要求结论部分**详细到能推动改进、讲完别让人懵**。结构 = **3 个角色**(`体验部`紫 / `资料内容`青 / `开发`蓝,把旧 6 部门并成 3)× **板块**(一页一个板块、详细展开)。**每页版式**(原生 HTML、非 iframe):① 页头 `.role-chip`(角色大色块)+ `.bk-strip`(该角色的几个板块、当前 `.on` 高亮)——角色和板块都要突出;② `.rgoal`(目标 banner,accent-soft,显式「目标」);③ `.ameasures`(措施 2 列网格、铺满)按**对象分三组** `.ahead`(`给「人」.hu`金 / `人+AI 共通.co`紫 / `给「AI」.ai`青),每条 `.rmi` = 优先级色块 `.rp`(P0红/P1橙/P2灰)+ **措施大字** `.rmi-act`(fs-h3)+ **分析依据小字** `.rmi-why`(带 `.src2` 触点/供给侧来源)。**铁律**(都是用户逐条打回攒的):**(a) 措施作大字、分析联系作小字**(看得出怎么从前面分析推出来);**(b) 不放 demo**——会局限、且和触点/供给侧页重复(用户:"前面画过了");**(c) 措施要全**,覆盖前面**所有** touchpoint `rec`(28 条,`ar-embed.html` 里 `rec:` 字段)+ 供给侧 8 发力点 + 七个问题,别只挑几条;**(d) 设计点(原①–⑤)化进具体措施**、别单列「升级/方向」(智能助手=设计点①、文档可对话可运行=②、沉浸学习=③、意图路径=④、社区知识 skill 化=⑤);**(e) 字号只用 ramp、严禁 emoji**(`📁` 被打回过)。角色→板块映射:体验部=官网与落地/下载与账号/学习与上手/智能化触点;资料=文档与版本/社区与内容;开发=机器可读出口/工具与工程。**已建**:下载与账号(体验部);**待建**:其余板块各一页 + 开头「三角色总览」+ 删旧 `?sec=actions`/`?sec=dept` 三页。措施数据源:`ACTIONS`(人侧 P0–P3)、各触点 `rec`、ux19 供给侧 8 卡、`DEPT_OF`(旧归属参考)。
