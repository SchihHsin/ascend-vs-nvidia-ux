...
- **右栏随高亮联动**(用户要的「聚到 AI↔昇腾→右边出矩阵评分分析、聚到人侧→出 ascend ux 分析」):iframe 每次 `applyHi()` 用 **postMessage** `window.parent.postMessage({__seqStep:step},'*')` 通知父页(**不能直接 `window.parent.__seqOnStep` —— file:// 跨框同源策略会静默挡掉**);deck 第3页 `window.addEventListener('message',…)` → `__seqOnStep(n)` 切 `.seq-side`(宽 35%,`overflow-y:auto`)里的 `.ss-pane[data-step=n]`。⚠ `.ss-pane` 自定义了 `display:flex`,会盖住 `hidden` 属性,必须补 `.ss-pane[hidden]{display:none}` 才能正确隐藏。**右栏要厚、要用材料里的图表/截图,别只放文字**:pane1(矩阵评分)放 `ai-usability-heat.png`(opknow 矩阵)+过程+结论;pane2(技术体验)放 `touchpoint-heat.png`(触点热力)+ **NVIDIA/昇腾 文档截图对比**(`nv-docs.png`/`asc-docs.png`,从 `materials/ascend-vs-nvidia/screenshots/07-*` 拷来并 `sips -Z 1100` 压过)+过程+结论;图都 `<a target=_blank>` 可点开原图。
- **图填满白卡**:原 letterbox 左右留白大,是图比卡窄(aspect<卡)。把 `seq-d3.html` 生命线间距拉宽到 gap≈300(`LX hu/ai/off/tool/sec=175/475/775/1075/1375`)、`Wv 1180→1440`,使图 aspect≈卡 aspect、横向铺满。关联说明:**AI↔昇腾/二手=AI 可用性矩阵打分(opknow)**、**人↔昇腾社区=触点体验(ascend-vs-nvidia-ux)**。
