...
- **⚠ 当前状态(2026-06-24,覆盖上面若干旧描述)**:
  ...
  - **🟢 结论已重做成「分角色行动方案」(2026-06-30,替换旧 行动建议+责任归属 iframe 页)**:用户要求结论部分**详细到能推动改进、讲完别让人懵**。现结构 = **三角色总览 1 页 + 3 个角色**(`体验部`紫 / `资料内容`青 / `开发`蓝,把旧 6 部门并成 3)× **8 个行动页**:体验部=官网与落地/下载与账号/学习与上手/**文档体验**;资料=**文档内容与版本**/社区与内容;开发=机器可读出口/工具与工程。**文档相关板块不能共用一页**:体验部负责界面、路径、查找、阅读、局部 FAQ、分诊与跳转;资料内容负责正文、版本口径、错误码、依赖矩阵、样例质量。`智能化触点`不是独立板块,只是搜索/FAQ/助手/分诊等横向能力,已拆进文档体验、社区与内容、机器可读出口等真实板块。旧 `ar-embed.html?sec=actions`、`ar-embed.html?sec=dept&pg=1/2` 三页已从 deck 删除,但历史 embed 能力可保留。
