...
- **⚠ 当前状态(2026-06-24,覆盖上面若干旧描述)**:
  ...
  - ⚠ **sec 嵌入页的 `.wrap` 限宽(2026-06-28)**:非 tponly 的 `?sec=` 嵌入沿用主报告 `.wrap{max-width:1180px;margin:0 auto}`——**文字类楼层(j1–j5/actions/dept)保持限宽利于阅读**,但 **journey(7 列 grid)会被限宽居中、两侧大片空白、显得窄**。修法:apply 里 `body.classList.add('sec-'+id)`,只对 `body.embed.sec-journey .wrap{max-width:none}` 解开,让 grid 铺满 iframe 右半区(别全局解开,会把文字楼层撑太宽)。journey 宽高比偏扁,铺满宽后纵向会留一点余白(正常 contain 行为)。
