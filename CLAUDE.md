...
- **深挖拆成「左图右内容」专页(用户最终要的形态)**:不是挤在一页右栏,而是**每个关系各拆几页,左侧统一是打着对应聚光灯的时序图、右侧是该页内容**。`seq-d3.html` 加 `?hi=N` 参数(`location.search` 读 → `go(N)` 锁定聚光灯 + 隐藏控制条),deck 各页 `<iframe src="seq-d3.html?hi=1/2">` 内嵌。左栏卡 **统一宽 `flex:0 0 36%`**(不一致会显得图大小不一)。
