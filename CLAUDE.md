...
- **⚠ 当前状态(2026-06-24,覆盖上面若干旧描述)**:
  ...
  - **结论行动页 tab / rgoal(2026-06-30)**:顶部板块 tab 已移到右上角 `.head-r.board-tabs`,替代原来的「行动页/板块」小字;只保留 tab,不放「负责的板块」说明。点击跳转靠 `.bk-strip[data-group="exp|doc|dev"]` 分组,不要再依赖说明文案。`div.rgoal` 右下角语义图标按 `data-board` 的 `1.1/1.2/.../3.2` 前缀匹配,别用旧 `01/02` 前缀,否则 mask 失效会露成方块。`rgoal` 左栏保持 30% 宽,不要为解决换行加宽挤压右侧。⚠ `rgoal` 是 flex column,正文必须包在 `.rgoal-t` 里;否则裸文本和 `<b>` 会各自成为 flex item,导致加粗词独占一整行。重点短语仍用黑色加粗 + 轻底色,`b` 保持普通 inline,不要做成 tag/inline-block/nowrap。
